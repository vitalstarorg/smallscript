...
class Test_Compiler2(SmallScriptTest):
    ...
    def test520_kwhead(self):
        ...
        ss = "tobj attr11"
        res = Method().interpret(ss).toPython().compile()(scope)
        ...
        method.toPython().compile()
        method.loglevel(4)      # suppress parameter mismatch error message.
        res = method(scope)
    ...
    def test600_block_closure(self):
        ...
        ss = "[:param1 | param1]"
        block = Method().interpret(ss).toPython().compile()
        closure = block(scope)
        ...
        method = Method().interpret(ss).toPython().compile(); block = method(scope)
